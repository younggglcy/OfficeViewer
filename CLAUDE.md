...
| File | Purpose |
|------|---------|
| `OfficeViewerApp.swift` | Entry point, AppDelegate, menu bar setup, Sparkle auto-update, file handling orchestration |
...
| `Services/FileDecoder.swift` | Extracts Office files using `/usr/bin/unzip`, formats XML with `/usr/bin/xmllint` |
| `Services/CommandRunner.swift` | Executes CLI commands via `/bin/zsh -c` |
| `Views/SettingsView.swift` | SwiftUI settings UI (`SettingsView`, `CommandRow`, `CommandEditor`) |
...
- `SUFeedURL` - Points to `appcast.xml` on main branch
- `SUPublicEDKey` - Public key for verifying signed updates

The `appcast.xml` is auto-generated by GitHub Actions on each release.
